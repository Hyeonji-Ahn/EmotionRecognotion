"""
Apply weight to vector by using kernel as weighting factor
1. get the magnitude of vector
2. get the kernel value from four points
"""
        ...
        for i, pt0 in enumerate(kwargs['point']):
            ...
            if np.isnan(pt0[0]) == False and np.isnan(pt0[1]) == False and np.isnan(pt1[0]) == False and np.isnan(pt1[1]) == False:
                if count%sparsity == 0:
                    ...
                    #HERE
                    print(pt0[0], " " , pt0[1], " ", disp_x, " ", disp_y, " ", pt1[0], " ", pt1[1])
...
def detect_face_landmarks(image_path, output_folder):
    ...
    # Return the points as a dictionary
    return [
...
def init(image_pattern, win_size_px, grid_size_px, result_file, area_of_intersest=None, Ref_First = False, *args, **kwargs):
    ...
    for j in range(len(points_in)):
        kernel = 0
        for k in range(4):
            matrix1 = [points_in[j][0]-center[k][0] , points_in[j][1]-center[k][1]] # [xi - ui, yi - vi]
            matrix3 = [[points_in[j][0]-center[k][0]],[points_in[j][1]-center[k][1]]]
            matrix4 = matrix1 * matrix2[k] * matrix3
            kernel += 0.25 * math.e ** (-0.5 * matrix4[0][0])
        kernels[j] = kernel
        ...
        if 'deep_flow' in kwargs:
            ...
            if image.mode in ('RGBA', 'LA'):
                ...
                for j in range(len(points_in)):
                        ...
                        else: #apply the weight
                            dx = final_point[j][0] - original_point[j][0]
                            dy = final_point[j][1] - original_point[j][1]
                            magVector = (dx ** 2 + dy **2) ** 0.5

                            #adding all kernel
                            #up right, up left, down left, down right
                            # sigma 1 , sigma 2, factor
                            
                            final_point[j][0] = final_point[j][0] * kernels[j] / magVector
                            final_point[j][1] = final_point[j][1] * kernels[j] / magVector
