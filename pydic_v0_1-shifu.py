...
#------------------------------------------------????????????????????????????????????????????????------------------------------------------------------
        if i == 0:
            ...
            disp = compute_disp_and_remove_rigid_transform(point_list[i], point_list[i-1])
            
            mygrid.add_raw_data(win_size, image_list[i-1], image_list[i], point_list[i-1], point_list[i], disp)

            disp_list.append(disp)
            mygrid.interpolate_displacement(point_list[i-1], disp, method=interpolation)
#---------------------------------------------------------------------------------------------------------------------
