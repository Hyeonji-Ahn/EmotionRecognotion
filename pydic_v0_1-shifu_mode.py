...
#################
def compute_statistics(numbers, outlier_percent=10, threshold=0.5):
    # First, we will remove all values below the threshold
    filtered_numbers = [num for num in numbers if (num >= threshold or num <= threshold * -1)]
    ...
    if(len(numbers_without_outliers) == 0):
        return 0,0,0
...
#WORKING
            ...
            dxs =  []
            dys = []
            for k in range(len(original_point)):
